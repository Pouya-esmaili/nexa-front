"use client"
import React, { useState } from 'react';

interface RatingProps {
    rate: number
    size?: number
    gap?: number
    isEdited?: boolean;
}

const Rating = ({ rate, size = 18, gap = 1, isEdited = false }: any) => {
    const [rating, setRating] = useState<number>(rate);
    const [hover, setHover] = useState<number>(0);
    const totalStars = 5

    const handleClick = (index: number) => {
        setRating(index);
    };

    if (!isEdited) {
        return (
            <div className={`flex flex-row gap-[${gap}px]`}>
                {Array.from({ length: totalStars }, (_, i) => i + 1).map((star) => (
                    <Star
                        key={star}
                        filled={star <= rating}
                        size={size}
                    />
                ))}
            </div>
        );
    }


    return (
        <div className={`flex flex-row gap-${gap}`}>
            {Array.from({ length: totalStars }, (_, i) => i + 1).map((star) => (
                <Star
                    key={star}
                    filled={star <= (hover || rating)}
                    onClick={() => handleClick(star)}
                    onMouseEnter={() => setHover(star)}
                    onMouseLeave={() => setHover(0)}
                    size={size}
                />
            ))}
        </div>
    );
};

interface StarProps {
    filled: boolean;
    onClick?: () => void;
    onMouseEnter?: () => void;
    onMouseLeave?: () => void;
    size: number
}

const Star: React.FC<StarProps> = ({ filled, onClick, onMouseEnter, onMouseLeave, size }) => {
    return (
        <span
            onClick={onClick}
            onMouseEnter={onMouseEnter}
            onMouseLeave={onMouseLeave}
            className="hover:cursor-pointer"
        >
            <svg
                className={`${filled ? "fill-yellow-400" : "fill-gray-400"}`}
                width={`${size}`}
                height={`${size}`}
                viewBox="0 0 24 24"
                xmlns="http://www.w3.org/2000/svg"
            >
                <path d="M11.1033 1.81699C11.4701 1.07374 12.5299 1.07374 12.8967 1.81699L15.294 6.67446C15.4397 6.9696 15.7213 7.17417 16.047 7.2215L21.4075 8.00043C22.2277 8.11961 22.5552 9.12759 21.9617 9.70612L18.0828 13.4871C17.8471 13.7169 17.7396 14.0479 17.7952 14.3723L18.7109 19.7111C18.851 20.528 17.9936 21.151 17.26 20.7653L12.4653 18.2446C12.174 18.0915 11.826 18.0915 11.5347 18.2446L6.74005 20.7653C6.00642 21.151 5.14899 20.528 5.2891 19.7111L6.20479 14.3723C6.26043 14.0479 6.15288 13.7169 5.91719 13.4871L2.03827 9.70612C1.44476 9.12759 1.77226 8.11961 2.59249 8.00043L7.95302 7.2215C8.27873 7.17417 8.5603 6.9696 8.70596 6.67446L11.1033 1.81699Z" />
            </svg>
        </span>
    );
};

export default Rating;