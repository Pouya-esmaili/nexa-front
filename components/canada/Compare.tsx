        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 max-w-[1100px] mx-auto">
          ...
          <Reveal variant="left" delay={100}>
            <div className="bg-white rounded-[28px] p-10 border border-[#E2E2E2]">
              ...
              <ul className="space-y-0">
                {diyList.map((item) => (
                  <li key={item.en} className="relative ps-8 py-3 text-[14.5px] leading-[1.5] border-b border-[#E2E2E2] last:border-b-0">
                    <span className="absolute start-0 top-3 w-[22px] h-[22px] rounded-full bg-[#F4F4F4] text-[#474747] flex items-center justify-center text-[12px] font-bold">
                      ✕
                    </span>
                    {t(item.en, item.fa)}
                  </li>
          ...
          <Reveal variant="right" delay={100}>
            <div className="rounded-[28px] p-10 relative bg-[#8F27FF]">
              {/* Recommended badge */}
              <div className="absolute top-6 end-6 text-[11px] font-bold tracking-[0.1em] px-3 py-1 rounded-full"
              ...
              <ul className="space-y-0">
                {nexaList.map((item) => (
                  <li key={item.en} className="relative ps-8 py-3 text-[14.5px] leading-[1.5] text-white"
                    style={{ borderBottom: "1px solid rgba(255,255,255,0.15)" }}>
                    <span className="absolute start-0 top-3 w-[22px] h-[22px] rounded-full bg-white flex items-center justify-center text-[12px] font-bold text-[#8F27FF]">
                      ✓
                    </span>
                    {t(item.en, item.fa)}
                  </li>
