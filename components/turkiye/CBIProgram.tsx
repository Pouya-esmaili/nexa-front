    ...
    <section className="py-16 md:py-20 bg-white">
      <Row>
        ...
        <div className="flex flex-col lg:flex-row gap-10">
          ...
          {/* Sidebar */}
          <aside
            className="lg:w-64 flex-shrink-0 rounded-2xl p-7 self-start flex flex-col gap-5"
          ...
          <ol className="flex-1 flex flex-col gap-0 relative">
            {steps.map((s, i) => (
              <li key={s.num} className="flex gap-5 relative">
                {/* Connector line */}
                {i < steps.length - 1 && (
                  <div className="absolute left-[19px] top-10 bottom-0 w-px bg-gray-100" />
                )}
                <div
                  className="flex-shrink-0 w-10 h-10 rounded-full flex items-center justify-center text-[13px] font-bold z-10"
                  style={
                    s.highlight
                ...
                <div
                  className={`flex-1 mb-6 p-5 rounded-2xl ${s.highlight ? "border-[1.5px] border-[#8F27FF]" : "border border-gray-100"}`}
