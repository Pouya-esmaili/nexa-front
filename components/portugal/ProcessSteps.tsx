          ...
          <div className="relative pl-14">
            ...
            {steps.map((s, i) => (
              <Reveal key={s.num} variant="up" delay={i * 100 + 100}>
                <div className="relative pb-8 last:pb-0 group transition-transform duration-250 hover:translate-x-1">
                  <div
                    className="absolute -left-14 top-0 w-[42px] h-[42px] rounded-full flex items-center justify-center text-[13px] font-bold text-[#8F27FF] bg-white transition-all duration-300 group-hover:bg-[#8F27FF] group-hover:text-white"
                    style={{ border: "2px solid #8F27FF", boxShadow: "0 0 0 5px #FAF6FF, 0 4px 12px rgba(143,39,255,0.12)" }}
                  >
                    {s.num}
                  </div>
                  <div className="bg-white border border-[#E2E2E2] rounded-[14px] px-5 py-4 transition-colors duration-200 group-hover:border-[rgba(143,39,255,0.18)]">
